...
//var_dump( $context['first_message']);
	...
	echo '
			...
			if ($context['first_message'] != $context['topic_first_message'] ){
				echo'
					...
					</h3><div class="centertext dataTopic"><span class="smalltext"><span class="',$context['class'],'">', $txt[$context['class']], '</span> <i class="icon-eye-1"></i>', $context['num_views'], ' <i class="icon-comment"></i>', $context['num_replies'], ' </span></div></div>';
				}
	...
	while ($message = $context['get_message']())
	{
		...
		echo '
				<div class="', $message['approved'] ? ($message['alternate'] == 0 ? 'windowbg' : 'windowbg2') : 'approvebg', '"> 
					<div class="post_wrapper ',$CheckD? 'firstPostT':'oTersPostT','">';
		...
		{ 
			...
			if($message['member']['is_guest']){
				...
				if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']))
					echo '
									<li class="avatar">
										<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '" ',$onlineuser? 'title="'.$message['member']['online']['text'].'"':'','><div class="avatar_online">
				', !empty($message['member']['avatar']['image']) ? $message['member']['avatar']['image']: '<img src="'.$settings['theme_url'].'/images/theme/default.png" alt="" class="avatar emtyAv"  data-avatar="'.$message['member']['name'].'"/>', '
										 ',$onlineuser? $message['member']['online']['is_online']?  '<span class="onlineuser"></span>':'<span class="offlineuser"></span>':'','
										</div></a>
					...
					echo'
						<div class="postarea">
							<div class="topicMenu">
								...
								<div class="keyinfo Keydhayzon" id="Dmsg_' . $message['id'] . '">';							
									
									echo'<h1 id="subject_', $message['id'], '" ',!$CheckD? ' style="display:none"':'','> 
										<a href="', $message['href'], '" rel="nofollow">', $message['subject'], '</a>
									</h1>';

								if ($msgopt) {
									 echo'
									 <a href="#" class="clicked floatright mArgin_t5" data-toggle="TopicButtons"><span class="dhazon_svg_icon utopicOptions"></span></a>';
								}
														// Is this user allowed to modify this member's karma?
				if ($message['member']['karma']['allow'] && !$context['user']['is_guest'])
					echo '
									<div class="karma_allow floatright">
							...
							if (!empty($options['display_quick_mod']) && $options['display_quick_mod'] == 1 && $message['can_remove'])
								echo '
									<div class="inline_mod_check floatright" style="display: none;" id="in_topic_mod_check_', $message['id'], '"></div>';
								...
								echo'
									...
									if (!$CheckD) {
										echo'<h3>'.$message['member']['link'].' <small> ', !empty($message['counter']) ? $txt['reply_noun'] . ' <i class="icon-hash"></i> ' . $message['counter'] : '', ' '.$message['time'].'</small></h3> ';
									} 

									if($CheckD)
									echo'<div class="floatright dataTopic"><span class="smalltext"><i class="icon-eye-1"></i>', $context['num_views'], ' <i class="icon-comment"></i>', $context['num_replies'], ' <span class="',$context['class'],'">', $txt[$context['class']], '</span></span></div>';

									echo'<div id="msg_', $message['id'], '_quick_mod"></div>
								';
		echo'<div class="TopicButtons" style="display:none">';
		...
		if ($message['can_approve'])
			echo '
									<li class="approve_button"><a href="', $scripturl, '?action=moderate;area=postmod;sa=approve;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '"><i class="icon-ok-1"></i> ', $txt['approve'], '</a></li>';

		// Can they reply? Have they turned on quick reply?
		if ($context['can_quote'] && !empty($options['display_quick_reply']))
			echo '
									<li class="quote_button"><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '" onclick="return oQuickReply.quote(', $message['id'], ');"><i class="icon-quote-right-alt"></i> ', $txt['quote'], '</a></li>';

		// So... quick reply is off, but they *can* reply?
		elseif ($context['can_quote'])
			echo '
									<li class="quote_button"><a href="', $scripturl, '?action=post;quote=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], ';last_msg=', $context['topic_last_message'], '"><i class="icon-quote-right-alt"></i> ', $txt['quote'], '</a></li>';

		// Can the user modify the contents of this post?
		if ($message['can_modify'])
			echo '
									<li class="modify_button"><a href="', $scripturl, '?action=post;msg=', $message['id'], ';topic=', $context['current_topic'], '.', $context['start'], '"><i class="icon-vector-pencil"></i> ', $txt['modify'], '</a></li>';

		// How about... even... remove it entirely?!
		if ($message['can_remove'])
			echo '
									<li class="remove_button"><a href="', $scripturl, '?action=deletemsg;topic=', $context['current_topic'], '.', $context['start'], ';msg=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '" onclick="return confirm(\'', $txt['remove_message'], '?\');"><i class="icon-archive"></i> ', $txt['remove'], '</a></li>';

		// What about splitting it off the rest of the topic?
		if ($context['can_split'] && !empty($context['real_num_replies']))
			echo '
									<li class="split_button"><a href="', $scripturl, '?action=splittopics;topic=', $context['current_topic'], '.0;at=', $message['id'], '"><i class=" icon-wrench"></i> ', $txt['split'], '</a></li>';

		// Can we restore topics?
		if ($context['can_restore_msg'])
			echo '
									<li class="restore_button"><a href="', $scripturl, '?action=restoretopic;msgs=', $message['id'], ';', $context['session_var'], '=', $context['session_id'], '"><i class="icon-quote-left-alt"></i> ', $txt['restore_message'], '</a></li>';
		...
		//show  autor poster info
		if($CheckD){

			echo'<div class="topicAutor">';
			...
			if (!$message['member']['is_guest'])
			{
				...
				echo'<div class="flow_hidden autorTopicInfo">';
			...
			if ($message['member']['karma']['allow'])
				...
				if (!empty($message['member']['group']))
					echo '
							<div class="membergroup floatright centertext">', $message['member']['group'], ' <div class="group_stars"> ', $message['member']['group_stars'], '</div></div>';


			 	echo'<h2>'.$message['member']['link'].'  ',$onlineuser ?  '<small>'.$message['member']['online']['text'].'</small>':'','</h2>
			 		<div class="smalltext">', $message['time'], '</div>';
				
				// Show the post group if and only if they have no other group or the option is on, and they are in a post group.
				if ((empty($settings['hide_post_group']) || $message['member']['group'] == '') && $message['member']['post_group'] != '')
					echo '
									<span class="postgroup"><i class="icon-star"></i> ', $message['member']['post_group'], '</span>';


				// Show how many posts they have made.
				if (!isset($context['disabled_fields']['posts']))
					echo '
									<span class="postcount">', $txt['member_postcount'], ': ', $message['member']['posts'], '</span>';

							// Is karma display enabled?  Total or +/-?
				if ($modSettings['karmaMode'] == '1')
					echo '
									<span class="karma">', $modSettings['karmaLabel'], ' ', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '</span>';
				elseif ($modSettings['karmaMode'] == '2')
					echo '
									<span class="karma">', $modSettings['karmaLabel'], ' +', $message['member']['karma']['good'], '/-', $message['member']['karma']['bad'], '</span>';
		...
		if ($message['can_modify'])
			echo '
							<div  class="modifybutton floatright" id="modify_button_', $message['id'], '" style="cursor: ', ($context['browser']['is_ie5'] || $context['browser']['is_ie5.5'] ? 'hand' : 'pointer'), ';" onclick="oQuickModify.modifyMsg(\'', $message['id'], '\')" ></div>';
		...
		if ($context['can_report_moderator'])
			echo '
								<a href="', $scripturl, '?action=reporttm;topic=', $context['current_topic'], '.', $message['counter'], ';msg=', $message['id'], '">', $txt['theme_report'], '</a>';
		...
		elseif (!$context['user']['is_guest'])
			echo '
								<a href="', $scripturl, '?action=helpadmin;help=see_member_ip" onclick="return reqWin(this.href);" class="help">(?)</a>';
		...
		if (!empty($settings['dhayzon_ads_display']) && $CheckD)
		echo'<div class="content centertext">',$settings['dhayzon_ads_display'],'</div>'; 
	}
	...
	if ($context['can_reply'] && !empty($options['display_quick_reply']))
	{
		echo '
			<a id="quickreply"></a>
			<div class="post_wrapper" id="quickreplybox">
			<div class="postarea">
				<div class="cat_bar centertext">
					<h3 class="catbg"> 
						<a href="#" id="Qreply_dhayzon" class="clicked btn_success" ><span class="dhazon_svg_icon dha_reply"></span> ', $txt['quick_reply'], '</a>						 
					</h3>
				</div>
				<div id="quickReplyOptions" ', $options['display_quick_reply'] == 2 ? ' class="collapse" ' : 'style="display:none"', '>
			...
			echo '
							<div class="quickReplyContent">
								<textarea name="message" tabindex="', $context['tabindex']++, '"></textarea>
							</div>
	...
	echo '
			...
			</div><div class="nextlinks">', $context['previous_next'], '</div>';

	// Show the lower breadcrumbs not nessesary.
	//theme_linktree();

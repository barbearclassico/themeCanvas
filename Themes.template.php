...
function template_set_settings()
{
	...
	<div id="admincenter">
		<form action="', $scripturl, '?action=admin;area=theme;sa=settings;th=', $context['theme_settings']['theme_id'], '" method="post" accept-charset="', $context['character_set'], '">
			...
			<div id="dhayzon-theme-settings">
					<div class="dhayzon_theme_options">
						<a href="#theme_edit">', $txt['theme_edit'], '</a>
						',!empty($context['theme_variants'])? '<a href="#theme_variants">'.$txt['theme_variants'].'</a>':'','
						 ';
						foreach ($context['settings'] as $setting){
							if (!empty($setting) && $setting['type'] == 'tab') 
								echo'<a href="#', $setting['id'],'">', $setting['label'],'</a>';

						}

						echo'
					</div>
				<div class="dhayzon_theme_content">
					<div id="theme_edit" class="dhayzon_theme_tab current">';
	...
	// Do we allow theme variants?
		if (!empty($context['theme_variants']))
			{
							echo '
							<div id="theme_variants" class="dhayzon_theme_tab">
								<div class="cat_bar">
									<h3 class="catbg">
										 ', $txt['theme_variants'], '
									</h3>
...
//  echo'</pre>';
		...
		foreach ($context['settings'] as $setting)
		{
			...
			if ( !empty($setting) && $setting['type'] == 'tab' )
			{
				echo '<div id="',$setting['id'],'" class="dhayzon_theme_tab"><div class="cat_bar"><h3>',$setting['label'],'</h3></div>';
			}
			...
			{
				echo '<dl class="settings flow_auto">
							<dt>
			...
			//text area by gary http://www.simplemachines.org/community/index.php?topic=481674.0

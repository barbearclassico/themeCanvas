...
function template_main()
{
	...
	<a id="top"></a>';
	 ...
	 if(!empty($settings['dhayzon_ads_messageTop']))
	 	echo'<div class="content centertext"> ',$settings['dhayzon_ads_messageTop'],'</div>';
	...
	<div class="tborder childboards" id="board_', $context['current_board'], '_childboards">
		...
		{
			echo '
				 <div id="board_', $board['id'], '" class="block-group">
					 <div class="icon block"', !empty($board['children']) ? ' ' : '', '>
						<a href="', ($board['is_redirect'] || $context['user']['is_guest'] ? $board['href'] : $scripturl . '?action=unread;board=' . $board['id'] . '.0;children'), '">';

			// If the board or children is new, show an indicator.
			if ($board['new'] || $board['children_new'])
				echo '
							<div class="boardIcon new_post ',$board['new'] ? '' : 'children_new', '"></div> ';
			// Is it a redirection board?
			elseif ($board['is_redirect'])
				echo '
							<div class="boardIcon is_redirect"></div>';
			...
			if (!empty($board['last_post']['id']))
				echo '
						...
						<small>', $txt['on'], ' ', $board['last_post']['time'],'</small>
						</p>';
			...
			if (!empty($board['children']))
			{
				...
				echo '
				 <div id="board_', $board['id'], '_children"> <div  class="children windowbg"><strong>', $txt['parent_boards'], '</strong>: ', implode(', ', $children), '</div></div>';
			}
		}
		echo '
				</div>
	...
	echo'<div class="centertext board_desc">';
	...
	if($context['can_post_new'])
		echo'<a class="btn-success" href="',$scripturl,'?action=post;board=',$context['current_board'],'.0">',$txt['new_topic'],'</a>';
	...
	<div  id="messageindex">
		 <div class="table_list">
			 <div class="order_by_dhayzon"> ';

		// Are there actually any topics to show?
		if (!empty($context['topics']))
		{
			echo '<div class="buttonlist floatleft">
			   <a href="#dhayzonTogleOptions" data-toggle="dhayzonTogleOptions" class="clicked btn-success">',$txt['theme_order_by'] ,'</a>

				<ul style="display:none"> <li><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=subject', $context['sort_by'] == 'subject' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['subject'], $context['sort_by'] == 'subject' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a>   <a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=starter', $context['sort_by'] == 'starter' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['started_by'], $context['sort_by'] == 'starter' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></li>
				...
				if (empty($context['can_quick_mod']))
					echo '<li><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a> </li>';
				else
					echo '<li>
						 <a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></li>';
		...
		// No topics.... just say, "sorry bub".
			...
			echo '
					 <div class="dhayzon_tb_0"><strong>', $txt['msg_alert_none'], '</strong></div>
				';
		...
		if (!empty($settings['display_who_viewing']))
		{
			echo '
				 <div class="windowbg2 whos_viewing">
					 <div colspan="', !empty($context['can_quick_mod']) ? '6' : '5', '" class="smalltext">';
		...
		if (!empty($context['unapproved_posts_message']))
		{
			echo '
				 <div class="windowbg2">
					 <div colspan="', !empty($context['can_quick_mod']) ? '6' : '5', '">
		...
		foreach ($context['topics'] as $topic)
		{
			...
			echo '
				 <div id="dmsg_' . $topic['id'] . '" class="block-group ', $color_class, '">
					 <div class="icon  block"> 
						<img src="', $topic['first_post']['icon_url'], '" alt="" />
						 <div><a href="', $topic['last_post']['href'], '"><i class="icon-paper-plane" title="', $txt['last_post'], '"></i> </a></div>
					</div>
					 <div class="info block">
						<div ', (!empty($topic['quick_mod']['modify']) ? 'id="topic_' . $topic['first_post']['id'] . '" onmouseout="mouse_on_div = 0;" onmouseover="mouse_on_div = 1;" ondblclick="modify_topic(\'' . $topic['id'] . '\', \'' . $topic['first_post']['id'] . '\');"' : ''), '> 

							<h2><span id="msg_' . $topic['first_post']['id'] . '"><span class="',$topic['class'],'">', $txt[$topic['class']], '</span> ', $topic['first_post']['link'], (!$context['can_approve_posts'] && !$topic['approved'] ? '&nbsp;<em>(' . $txt['awaiting_approval'] . ')</em>' : ''), '</span>
			...
			if ($topic['new'] && $context['user']['is_logged'])
					echo '
							<a href="', $topic['new_href'], '" id="newicon' . $topic['first_post']['id'] . '"><span class="normal_newpost new_posts">', $txt['new'], '</span></a>';

			echo '
							</h2>
							<div class="topic_stats_dhayzon">
							<span>', $txt['last_post'], ' ', $txt['by'], ' ', $topic['last_post']['member']['link'], ' </span>
							<span><small>', $topic['last_post']['time'], '</small></span>';

							if(!empty($topic['pages']))
							echo'<div id="pages' . $topic['first_post']['id'] . '">', $topic['pages'], '</div>';

							echo'</div>
						</div>
					</div> 
					 <div class="lastpost block floatright ">';
			...
			if (!empty($context['can_quick_mod']))
			{
				...
				echo '
					 ...
					 echo'<span>', $txt['started_by'], ' ', $topic['first_post']['member']['link'], ' </span>
					 <div class="topic_stats_dhayzon">
						
						 <span>', $topic['views'], ' ', $txt['views'], ' 	', $topic['replies'], ' ', $txt['replies'], '</span>
	...
	// Show breadcrumbs at the bottom too. not nessesary 
	//theme_linktree();
 	//delete jumpTo
